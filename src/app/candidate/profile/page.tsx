  ...
  return (
    <main className="max-w-4xl mx-auto py-8 px-4 sm:py-12">
      <div className="mb-6 sm:mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <h1 className="text-3xl sm:text-4xl font-bold text-[#0D1B2A]">Your Profile</h1>
        {!editing && (
          <button
            onClick={() => setEditing(true)}
            className="px-4 py-2 bg-[#D4AF37] text-[#0D1B2A] rounded-lg font-medium hover:bg-[#c49d23] transition text-sm sm:text-base"
          >
            Edit Profile
          </button>
        )}
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-lg shadow-md p-6 sm:p-8 space-y-6 mb-8">
          {/* Full Name */}
          <div>
            <label className="block text-xs sm:text-sm font-medium text-[#333333] mb-2">Full Name</label>
            {editing ? (
              <input
                type="text"
                value={fullName}
              ...
              <p className="text-[#0D1B2A] font-medium text-sm sm:text-base">{profile.full_name}</p>
            )}
          </div>

          {/* Email */}
          <div>
            <label className="block text-xs sm:text-sm font-medium text-[#333333] mb-2">Email Address</label>
            <p className="text-[#0D1B2A] font-medium text-sm sm:text-base">{profile.email}</p>
            <p className="text-xs text-gray-500 mt-1">Contact your recruiter to change your email</p>
          </div>

          {/* Bio */}
          <div>
            <label className="block text-xs sm:text-sm font-medium text-[#333333] mb-2">Professional Bio</label>
            {editing ? (
              <textarea
                value={bio}
              ...
              <p className="text-[#0D1B2A] text-sm sm:text-base">{bio || 'No bio provided'}</p>
            )}
          </div>

          {/* Skills */}
          <div>
            <label className="block text-xs sm:text-sm font-medium text-[#333333] mb-2">Skills</label>
            {editing ? (
              <textarea
                value={skills}
              ...
              <div className="flex flex-wrap gap-2">
                {profile.skills.length > 0 ? (
                  ...
                  <p className="text-gray-600 text-sm">No skills added</p>
                )}
              </div>
            )}
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        ...
        >
          <svg
            className="w-10 sm:w-12 h-10 sm:h-12 mb-4 text-[#D4AF37]"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
          ...
          <p className="text-xs sm:text-sm text-gray-600">Upload or replace your CV</p>
        </Link>
        ...
        >
          ...
          <p className="text-xs sm:text-sm text-gray-600">Manage account settings</p>
        </Link>
      </div>
    </main>
  )
