...
export default function AdminLoginPage() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      console.log('[Admin Google Login] Initiating Google OAuth')
      ...
      if (error) {
        console.error('[Admin Google Login] OAuth error:', error)
        setError(error.message)
        setLoading(false)
      }
    } catch (err) {
      console.error('[Admin Google Login] Unexpected error:', err)
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('[Admin Forgot Password] Sending reset email to:', forgotEmail)
            ...
            <div className="w-full max-w-md bg-white rounded-lg shadow-lg p-8">
              ...
              {!forgotMessage.includes('sent') && (
                <form onSubmit={handleForgotPassword} className="space-y-4 mb-6">
                  <div>
                    <label htmlFor="adminForgotEmail" className="block text-sm font-medium text-[#333333] mb-1">
                      Email Address
                    </label>
                    <input
                      type="email"
                  ...
                  <button
                    type="submit"
                    disabled={forgotLoading}
                    className="w-full bg-[#D4AF37] text-[#0D1B2A] py-2 rounded-lg font-medium hover:bg-[#c49d23] disabled:opacity-50 transition"
                  >
